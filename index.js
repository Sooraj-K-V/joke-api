...
app.get("/random", async (req, res) => {
  try {
    ...
    res.json(jokes[randomNumber].jokeText);
  } catch (error) {
...
app.post("/jokes", async (req, res) => {
  try {
    ...
    const idd = jokes.length + 1;
    console.log(joke, type, idd);
    const newJoke = {
      id: idd,
      jokeText: joke,
      jokeType: type,
...
app.patch("/jokes/:id", async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const editingJoke = jokes.find((joke) => joke.id === id);
    const replace = {
      id: id,
      jokeText: req.body.joke || editingJoke.jokeText,
      jokeType: req.body.type || editingJoke.jokeType,
    };
    const searchIndex = jokes.findIndex((joke) => joke.id === id);
    jokes[searchIndex] = replace;
...
app.delete("/delete/:id", async (req, res) => {
  try {
    ...
    const jokeToDelete = jokes.find((joke) => joke.id == idToDelete);
    const indexOfItem = jokes.findIndex((joke) => joke.id == idToDelete);
    jokes.splice(indexOfItem, 1);
